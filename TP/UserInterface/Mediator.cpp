...
//										break;
			case IndexWrapper::FECHA:
														clave = Utility::intToString(book.getWordCount());
														cout << "Book begin process: " << book.getTitle() << " - Fecha:" << clave << " - Start:" << Utility::getDate()<<endl;
...
string  Mediator::searchInternalID(string identificator){
	...
	bookIds = this->indexWrapper->searchAllIds(k, IndexWrapper::TITULO);

	return Utility::intToString(bookIds->front());
}
